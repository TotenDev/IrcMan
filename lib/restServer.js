...
restServer.prototype.authenticateRequest = function authenticateRequest(req,res,next) {
  //Check
  if(!req.headers["authorization"]) { res.send(401,{error:"Unauthorized."}); }
  else { 
    //Try to parse auth
    ...
    if (auth.length >= 2) {
      ...
      return next();
    }else { res.send(401,{error:"Unauthorized."}); return; }
  }
}
...
restServer.prototype.routeRequest = function routeRequest(req, res, next) {
  ...
  req.on('end',function () {
    if (!errored && !restServerInstance.route(req.getPath(),data,restServerInstance.getClientIp(req),res)) {
