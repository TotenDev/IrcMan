      ...
      if (path == '/postMessage' || path == '/postMessage/') {
        ...
        try {
          ...
          if (obj["startDate"] && obj["endDate"]) {
            //
            var startRange = new Date(obj["startDate"] + " 00:00:00"),
                endRange = new Date(obj["endDate"] + " 23:59:59");
            startRange = startRange.getFullYear() + "-" + (startRange.getMonth()+1) + "-" + startRange.getDate() ;
            endRange = endRange.getFullYear() + "-" + (endRange.getMonth()+1) + "-" + endRange.getDate() ;
            //
...
IrcMan.prototype.searchMessagesBetween = function searchMessagesBetween(startDate,endDate,callback) {
   ...
   this.defaultDatabaseConnection(function (connection) {
     if (!responsed) {
      var strQuery = util.format('SELECT * FROM `%s` WHERE `%s` BETWEEN \'%s\' AND \'%s\'','messages','timeStamp',startDate,endDate);
